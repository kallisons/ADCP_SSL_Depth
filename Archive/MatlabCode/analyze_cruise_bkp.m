...
cout.ampday   = nan(size(depth));
cout.ampdiff  = nan(size(depth));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     - day/night starts/ends abruptly
 ...
 for indd=1:nday
    if igday(indd)==1
       ...
       ampin = adcp_clean_amp(cruise.amp(:,locin),cruise.depth(:,locin),cruise.pflag(:,locin),'flag',A.flag,'blank',A.blank); 
       ...
       if igoodbef==1
          ...
          ampbef = adcp_clean_amp(cruise.amp(:,locbef),cruise.depth(:,locbef),cruise.pflag(:,locbef),'flag',A.flag,'blank',A.blank); 
          % Night Before
       ...
       if igoodnex==1
          ...
          ampnex = adcp_clean_amp(cruise.amp(:,locnex),cruise.depth(:,locnex),cruise.pflag(:,locnex),'flag',A.flag,'blank',A.blank);  
          % Next night
       ...
       cout.ampday(:,kday)	= profampin;
       if (igoodbef==1 | igoodnex==1)
          % At least one night around daytime are good
          cout.idiff(kday) 	= 1;
          cout.ampdiff(:,kday) = profampin - nanmean([profampnex profampbef],2);
       else
          % No nights around daytime  
          cout.idiff(kday) 	= 0;
          cout.ampdiff(:,kday) = nan(size(profampin));
       end
